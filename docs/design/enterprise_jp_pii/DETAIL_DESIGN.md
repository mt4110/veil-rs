...
- [x] Offline RulePack signature verification（`pinned_digests` / `sha256`）
- [x] RulePack update flow（`implementation/rulepack_update_flow.md`）
- [x] Enterprise policy templates（`templates/policies/*.toml`）
- [x] Evidence signing playbook（`implementation/evidence_signing_playbook.md`）
