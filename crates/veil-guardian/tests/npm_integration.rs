...
fn test_npm_integration() {
    ...
    // HACK: We can't easily inject the base URL into OsvClient without changing its signature significantly
    // or using an env var. For this test to work without deeper refactoring, we might need
    // to make the URL configurable via env var in client.rs.
    // Let's assume user is okay with us adding an ENV var override to `client.rs`.
    // OR, we can't test full integration easily without that.
    // Plan B: The user asked for "wiremock (dev)".
    // Let's modify client.rs to read OSV_API_URL env var or default.
    // Configure env var
    unsafe {
        std::env::set_var("OSV_API_URL", mock_server.uri() + "/v1/querybatch");
    }
    ...
    // 3. Scan
    let result = scan_lockfile(&lock_path, ScanOptions { offline: false }).expect("Scan failed");
